-- Email Microwave Database Clean Fix
-- Run this in your Supabase SQL Editor to completely reset and fix the database

-- Step 1: Drop everything that might conflict
...
-- Step 6: Create user_profiles table (references auth.users)
...
-- Step 7: Create emails table (references auth.users)
...
-- Step 11: Create policies for user_profiles
...
-- Step 12: Create policies for emails
...
SELECT 
    'Database clean fix completed successfully!' as status,
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'user_profiles' AND table_schema = 'public') as user_profiles_table_exists,
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'emails' AND table_schema = 'public') as emails_table_exists; 